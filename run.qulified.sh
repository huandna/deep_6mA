...
ln -s /home/yaoxinw/bacteria/process/${bac}/modifications_new.gff ${wkd}
ln -s /home/yaoxinw/bacteria/process/${bac}/modifications_new.csv ${wkd}
...
grep -F -vf /home/yaoxinw/bacteria/process/${bac}/err_reads_0.4.txt ${wkd}/all_ipd.sam > ${wkd}/all_ipd2.sam
#mv ${wkd}/all_ipd2.sam ${wkd}/all_ipd.sam
mkdir -p ${wkd}/split_sam
split -d -n l/100 ${wkd}/all_ipd2.sam ${wkd}/split_sam/splitsam
...
cat modifications_new.gff | grep -v "##" - | sed 's/ /\t/g' | sed 's/coverage=//' | sed 's/;IPDRatio=/\t/' | sed 's/;context=/\t/' | sed 's/;frac=/\t/' | sed 's/;fracLow=/\t/' | sed 's/;fracUp=/\t/' | sed 's/;identificationQv=/\t/' | sed 's/\t/,/g' > m6A.gff
cat ${wkd}/modifications_new.csv > modifications.csv
...
for((i=1;i<=5000000;i=i+50000));do end=$(echo $i+50000 | bc);echo $i $end;done > ${wkd}/region_list
...
cat ${wkd}/*coding1.csv > positive_data2
cat ${wkd}/*coding0.csv > negative_data2
