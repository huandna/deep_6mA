		...
		if identiy >= 0.75 and reads1.query_alignment_length >=50:
			...
			for i in range(len(pos_pair)):
				if reads_pos[i]-10>=0 and reads_pos[i]+11<=len(reads1.seq):
					...
					#reads_context_ipd="-".join(ip[reads_pos[i]-10:reads_pos[i]+11])
					#reads_context_pw="-".join(pw[reads_pos[i]-10:reads_pos[i]+11])
					reverse_tag= 1 if reads1.is_reverse else 0
					print(reads1.reference_name,ref_pos[i],reverse_tag,ip[reads_pos[i]],pw[reads_pos[i]],file=g)		
				else:
					continue
